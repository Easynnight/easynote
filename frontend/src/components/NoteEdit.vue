  ...
  <div class="note-edit-app">
    ...
    <header class="app-header">
      ...
      <div class="header-actions">
        <button @click="deleteNote" class="delete-btn">删除</button>
    ...
    <main class="note-edit-content">
      ...
      <div v-else-if="error" class="error">
        <p>{{ error }}</p>
        <button @click="fetchNoteData">重试</button>
      </div>
      ...
      <div v-else class="note-edit-form">
        ...
        <div class="form-footer">
          <div class="note-meta">
            <span class="created-at">创建于：{{ formatDate(note.createdAt) }}</span>
            <span v-if="note.updatedAt" class="updated-at">更新于：{{ formatDate(note.updatedAt) }}</span>
          </div>
...
export default {
  ...
  methods: {
    async fetchNoteData() {
      ...
      this.error = null;
      try {
        const response = await window.fetchWithAuth(`/api/notes/${noteId}`, {
          credentials: 'include'
        });
        if (!response.ok) throw new Error('获取笔记数据失败');
        const data = await response.json();
        this.note = data;
      } catch (error) {
        console.error('Error fetching note data:', error);
        this.error = '获取笔记数据失败: ' + error.message;
      } finally {
        this.loading = false;
      }
    },
    ...
    async saveNote() {
      ...
      try {
        const response = await window.fetchWithAuth(`/api/notes/${this.note.noteId}`, {
          method: 'PUT',
          headers: {
    ...
    async deleteNote() {
      if (confirm('确定要删除这条笔记吗？删除后将无法恢复。')) {
        try {
          const response = await window.fetchWithAuth(`/api/notes/${this.note.noteId}`, {
            method: 'DELETE',
            credentials: 'include'
